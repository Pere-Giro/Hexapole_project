...
Flag='ODE45';%Flag to define what you want the script to do 
%               (Flag == ODE)> integrate with ode45
...
Flag0='YES'; %Flag to define if you want to run the simulation with control law or not
            % (Flag0 == YES)> simulate with control law
            % (Flag0 == NO)> simulate without control law
...
%          xq=x0;
         ...
         switch Flag0
             case 'NO'
                 dxdt= @(t,xtheta) xdot_MXcord(t, xtheta, m, utaustar); %(t, xtheta, m, utaustar, xthetastar, K)
             case 'YES'
                 dxdt= @(t,xtheta) xdot_MXcord(t, xtheta, m, utaustar, xthetastar, K);
         end
         
         [times,states]=ode45(dxdt,t,xtheta0); %(dxdt,t,xtheta0,opts)
...
% %         
    ...
    case 'VIDEO'   
        ...
        % Set up the movie.
        writerObj = VideoWriter('hexapole_odestep','MPEG-4'); % Name it.
